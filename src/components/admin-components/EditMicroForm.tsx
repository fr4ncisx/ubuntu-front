...
import { EditMicroFormProps } from "../../models/InterfaceModels";
...
const EditMicroForm: React.FC<{
  ...
  const [microName, setMicroName] = useState(microemprendimiento?.nombre || "");
  const [category, setCategory] = useState<{ nombre: string }>(
    microemprendimiento?.categoria || { nombre: "" }
  );
  const [openCategory, setOpenCategory] = useState(false);
  const [categories, setCategories] = useState<string[]>([]);
  const [subcategory, setSubcategory] = useState(
    microemprendimiento?.subcategoria || ""
  );
  const [countries, setCountries] = useState<string[]>([]);
  const [selectedCountry, setSelectedCountry] = useState(
    microemprendimiento?.pais || ""
  );
  const [provinces, setProvinces] = useState<string[]>([]);
  const [openCountry, setOpenCountry] = useState(false);
  const [state, setState] = useState(microemprendimiento?.provincia || "");
  const [openState, setOpenState] = useState(false);
  const [city, setCity] = useState(microemprendimiento?.ciudad || "");
  const [description, setDescription] = useState(
    microemprendimiento?.descripcion || ""
  );
  const [info, setInfo] = useState(microemprendimiento?.masInformacion || "");
  const [images, setImages] = useState<File[]>([]);
  const [imageUrls, setImageUrls] = useState<string[]>(
    microemprendimiento?.imagenes.map((img: { url: string }) => img.url) || []
  );
  ...
  const microId = microemprendimiento?.id;
  ...
  const handleEditMicro = async () => {
    ...
    try {
      ...
      console.error("Error al editar el microemprendimiento:", error);
      setIsSuccess(false);
      setIsModalOpen(true);
    } finally {
  ...
  return (
    <>
      ...
      <Modal
        ...
        onRetry={!isSuccess ? handleEditMicro : undefined}
      />
    </>
  );
};

export default EditMicroForm;
